...
- `idealClass`, its multiplicativity and triviality criterion;
- the moving lemma and `idealClass_surjective`;
- `classMap`, its surjectivity, composition, and compatibility with `idealClass`;
...
`RayClassGroup (ratModulus n) ≃* (ZMod n)ˣ` and its compatibility with Mathlib's cyclotomic Galois
