...
- dimension, `discr`, `signedDiscr`, and later `hasseInvariant` and `localHasse`
  descend to it, in each case by the descent corollary of Layer 0;
...
the Witt index**; **Witt's chain-equivalence theorem**, which is what makes every
invariant of diagonal tuples well defined on isometry classes; the **representation
...
- **Representation and value sets.** `Represents Q a` and `unitValueSet Q` as pinned
  ...
  regular `Q` and `a : Kˣ`, `a ∈ unitValueSet Q` iff `Q ⊥ ⟨−a⟩` is isotropic. That
  criterion is what turns every later "which values does `q` represent" question
  into an isotropy question.
...
- **Chain equivalence.** This is the theorem every diagonal invariant's
  ...
  - `DiagonalStep := PermutationStep ⊔ BinaryStep`, and
  ...
  containment as a lemma and keep both, because permutation invariance is what most
  downstream proofs actually apply.)
...
- **The descent corollary**, which is the form every later layer uses: a function
...
deliberate: it is what makes each equivalence reusable. Nothing in this layer needs
central simplicity, and no milestone here assumes it.
...
- **Discriminant and signed discriminant** on `RegularFormClass K`: well-definedness
  ...
  spaces by `basisRepr`) or, equivalently, by the descent corollary applied to
...
- **Chain induction, prepared.** The two lemmas that Layer 5 and Layer 8 will feed
  into the descent corollary: a function of the shape `w ↦ ∏_{i<j} F (w i) (w j)`
  ...
  commutative monoid `M` and an abstract `F : Kˣ → Kˣ → M` is what lets Layer 5
...
- **The possible defects of a unit** (O'Meara 63:2, with the local square theorem
  ...
  defects of `1`, `5`, and `−1` respectively. This finite case list is what makes
  the dyadic computation below terminate.
...
- **Representation**, as a corollary rather than a vague reference: for regular `q`
  ...
  right-hand side is decided by the rank list above applied to
  `(dim q + 1, −a·d(q), s(q)·(−a, d(q))_K)`. Spell out the resulting description of
...
- **The fifth equivalent condition.** `(a) ∪ (b) = 0` in `H²(G_K, μ₂)` iff the four
  ...
  computation plus the four-fold criterion, and it is what completes B11a's
...
- `w₀ = 1`; `w₁(q) = (d(q))` with the **plain** discriminant, per the convention
  table; `w₂` of a diagonal form is `∑_{i<j} (aᵢ)(aⱼ)`, so `ι(s(q)) = w₂(q)`
  immediately from 7A, both sides being defined on a diagonalization.
...
- **Which functional.** For `L/K` finite, the nonzero elements of `Hom_K(L,K)` form a
  ...
  `s'(x) = s(λ x)`. Prove this first; it is what makes the change-of-functional
  theorem compare *every* two choices rather than a chosen family. For `L/K` finite
...
- Chain equivalence in one instance: `⟨1,1⟩ ≅ ⟨2,2⟩` over `ℚ` (both represent `2`,
  ...
  descent corollary is applied at least once on a form that is not diagonal in the
  obvious way (Layer 0).
