...
- **Tau Ceti material already merged.** `TauCeti/NumberTheory/Multiquadratic/Galois/*` and
  ...
  `IsArithFrobAt`, as Layer 5 does. Generalize those lemmas rather than duplicate them.

## Boundary with the number field roadmaps

Layer 5 of this roadmap proves Dedekind's factorization theorem on the polynomial side, from
Mathlib. The statement is about a polynomial, its reductions, and a permutation of its roots, so
it is stated in the vocabulary of this roadmap.

Ramification theory of number fields is not in this roadmap: the different, the relative
discriminant ideal, decomposition fields, inertia fields, the Artin symbol on ideals, and the
tame and wild exponents. A roadmap for number field arithmetic owns that material. Both
developments use Mathlib's `IsArithFrobAt`, which is the shared foundation.
