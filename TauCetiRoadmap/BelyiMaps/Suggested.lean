...
/-- **Layer 3.1.** The block generated by a pair, computably: close `{i, j}` under both
generators. -/
def blockOf (t : PermutationTriple n) (i j : Fin n) : Finset (Fin n) :=
  (orbitStep t)^[n] {i, j}

/-- **Layer 3.1.** Primitivity, computably: no pair of distinct labels generates a proper
block. ⚠ `1 < n` is guarded explicitly — on `Fin 0` and `Fin 1` there is no pair to scan, so
an unguarded scan reports `true` on the degenerate degrees. -/
def isPrimitiveB (t : PermutationTriple n) : Bool :=
  decide (1 < n) && decide (∀ i j : Fin n, i ≠ j → blockOf t i j = Finset.univ)
...
open scoped Classical in
/-- **Layer 3.1.** The passport fiber: the classes whose cycle data and monodromy group
match a specification. The condition is relabeling-invariant, so testing one member of a
class is testing all of them — which is itself a target below. -/
noncomputable def passportFiber (n : ℕ) (P : PassportSpec n) :
    Finset (Finset (PermutationTriple n)) :=
  (isoClasses n).filter fun c => ∃ t ∈ c,
    computedCycleType t.σ0 = P.lam0 ∧ computedCycleType t.σ1 = P.lam1 ∧
      computedCycleType t.σinf = P.laminf ∧
      ∃ τ : Equiv.Perm (Fin n), (monodromyGroup t).map (MulAut.conj τ).toMonoidHom = P.G
...
open scoped Classical in
/-- **Layer 3.1.** `passportSize` as a cardinality of computed data, with the theorem that
it counts isomorphism classes — what Layer 3.4's normalizer formula is checked against and
what Layer 14's `pass_size` certificate consumes. -/
noncomputable def computedPassportSize (n : ℕ) (P : PassportSpec n) : ℕ :=
  (passportFiber n P).card

open scoped Classical in
theorem computedPassportSize_eq_card {n : ℕ} (P : PassportSpec n) :
    computedPassportSize n P =
      Nat.card {c : IsoClass n // ∃ t : ConnectedTriple n,
        Quotient.mk _ t.1 = c ∧ PassportSpec.HasPassport t P} := by
  sorry
